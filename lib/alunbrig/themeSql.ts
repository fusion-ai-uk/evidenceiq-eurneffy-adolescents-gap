import type { AlunbrigThemeFilters, AlunbrigThemeGroupBy, AlunbrigThemeMetric } from "@/lib/alunbrig/themeFilters"
  ...
  return `
    WITH base AS (
      ...
      WHERE ${truthy("data_quality_keep_for_analysis")} = TRUE
        ...
        AND (@stakeholderPrimary IS NULL OR ARRAY_LENGTH(@stakeholderPrimary) = 0 OR stakeholder_primary IN UNNEST(@stakeholderPrimary))
...
export function getBaseParams(filters: AlunbrigThemeFilters) {
  const flagParams = flagsToParams(filters.flags)
  return {
    // Dates as strings; SQL casts via DATE(@startDate)/DATE(@endDate)
    startDate: filters.startDate,
    endDate: filters.endDate,
    includeLow: filters.includeLowRelevance,

    stakeholderPrimary: filters.stakeholderPrimary,
...
export function groupByToLabel(groupBy: AlunbrigThemeGroupBy): string {
  if (groupBy === "card_bucket") return "Card bucket"
  if (groupBy === "topics_top_topics") return "Top topics"
  if (groupBy === "clinical_context_biomarker") return "Biomarker / clinical context"
  return "Competitive context"
}

export function metricToLabel(metric: AlunbrigThemeMetric): string {
  if (metric === "volume") return "Posts"
