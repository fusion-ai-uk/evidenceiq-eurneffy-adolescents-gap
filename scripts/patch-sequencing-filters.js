const fs = require('fs');
const p = 'components/alunbrig/sequencing/SequencingExplorer.tsx';
...
// Add discard/dirty + chips helpers
...
// Replace the top Controls Card with FilterPane
const marker = '      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">';
const cardStart = s.indexOf('      <Card className="border-border/50">');
const markerIdx = s.indexOf(marker);
if (cardStart === -1 || markerIdx === -1 || markerIdx < cardStart) throw new Error('Could not locate sequencing filter card region');
...
s = s.slice(0, cardStart) + replacement + s.slice(markerIdx);
