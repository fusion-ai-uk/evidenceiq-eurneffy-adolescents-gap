...
export async function GET(req: Request) {
  ...
  const sql = `
    ...
    term_counts AS (
      SELECT grp_key, term, COUNT(*) AS count
      FROM term_rows
      GROUP BY grp_key, term
    ),
    term_top AS (
      SELECT
        grp_key,
    ...
    FROM grouped g
    LEFT JOIN stakeholder_top s USING(grp_key)
    LEFT JOIN term_top t USING(grp_key)
