...
type AlertRow = { category: string; current_volume: number; baseline_volume: number; pct_change: number }

export function BaselineAlerts() {
  const [rows, setRows] = useState<AlertRow[]>([])

  useEffect(() => {
    fetch("/api/timeseries/alerts?minBaseline=10&limit=5")
      .then((r) => r.json())
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    <div className="mt-2 text-xs text-muted-foreground/90">
                      {direction === "up" ? (
                        <span>
                          Rising interest — consider leaning in with context and content; watch sentiment on related safety/access threads.
                        </span>
                      ) : (
                        <span>
                          Cooling off — opportunity to re‑seed messaging or deprioritize unless strategic.
                        </span>
                      )}
                    </div>
                  </div>
